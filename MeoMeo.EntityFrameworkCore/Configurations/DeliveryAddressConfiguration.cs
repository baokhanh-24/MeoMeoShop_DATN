...
namespace MeoMeo.EntityFrameworkCore.Configurations
{
    public class DeliveryAddressConfiguration : IEntityTypeConfiguration<DeliveryAddress>
    {
        public void Configure(EntityTypeBuilder<DeliveryAddress> builder)
        {
            ...
            builder.HasOne(x => x.Province).WithMany(x => x.DeliveryAddresses).HasForeignKey(x => x.ProvinceId).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(x => x.District).WithMany(x => x.DeliveryAddresses).HasForeignKey(x => x.DistrictId).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(x => x.Commune).WithMany(x => x.DeliveryAddresses).HasForeignKey(x => x.CommuneId).OnDelete(DeleteBehavior.NoAction);
        }
    }
}
