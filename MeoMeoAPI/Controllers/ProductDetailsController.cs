    ...
    namespace MeoMeo.API.Controllers
    {
...

        {
            ...
            {   
                ...
                var lstMediaToUpload = productDetail.Images?.Select(c => c.UploadFile).ToList() ?? new List<IFormFile>();
            ...
            public async Task<IActionResult> Update(Guid id, [FromForm] CreateOrUpdateProductDetailDTO productDetail)
            {
                ...
                var newImages = productDetail.Images?.Where(img => img.Id == null).Select(img => img.UploadFile).ToList();
